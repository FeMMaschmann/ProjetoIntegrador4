 <?php
	Class conectar{
		...
		// cadastrar mensagens
		public function incluirMensagem($sql){
			$inserir_linha = $this->conn->query($sql) or die($this->conn->error.__LINE__);
			if ($inserir_linha) {
				header("Location: ../q&a.php?msg=Incluído!");
				$conn->close();
				exit();
			} else {
				die("Erro ao cadastrar:(".$this->conn->errno.")".$this->conn->error);
		...
		public function apagarMens($sql){
			$apagar_linha = $this->conn->query($sql) or die($this->conn->error.__LINE__);
			if ($apagar_linha) {
				header("Location: ../q&a.php?msg=Mensagem apagada com sucesso!");
				$conn->close();
				exit();
			} else {
				die("Erro ao apagar:(".$this->conn->errno.")".$this->conn->error);
