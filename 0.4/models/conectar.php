 <?php
	Class conectar{
		...
		public function cadastrarimg($sql){
			...
			if ($inserir_linha) {
				header("Location: galeria.php?msg=Dados cadastrados com sucesso!");
				$conn->close();
				exit();
			} else {
		...
		public function atualizar($sql){
			...
			if ($atualizar_linha) {
				header("Location: gerenciar_produtos.php?msg=Dados atualizados com sucesso!");
				$conn->close();
				exit();
			} else {
